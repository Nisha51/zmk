...
static void keymap_delayed_settings_load(struct k_work *_work) { settings_load_subtree("keymap"); }

static K_WORK_DELAYABLE_DEFINE(keymap_settings_load_work, keymap_delayed_settings_load);

int keymap_init(void) {
    settings_subsys_init();

    k_work_schedule(&keymap_settings_load_work, K_MSEC(100));
